...
export function CategoryFilters() {
  ...
  return (
    <div
      className="flex w-full overflow-x-auto no-scrollbar gap-3 py-2 items-center"
      style={{
        maskImage:
      ...
      {categoryTypes.map((t) => {
        ...
        return (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className={cn(
              "relative flex items-center gap-2.5 whitespace-nowrap rounded-full px-4 py-2.5 md:px-5 md:py-3 text-xs md:text-sm font-bold transition-all duration-300 flex-shrink-0 font-body overflow-hidden",
              isActive
                ? "text-white shadow-lg shadow-secondary/20"
                : "bg-white/10 text-white border border-white/20 hover:bg-white/15 shadow-sm backdrop-blur-md",
            )}
          >
            {isActive && (
              <motion.div
...
export function CityFilters() {
  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-4 md:gap-6">
      <div
        className="flex items-center gap-4 md:gap-6 w-full md:w-auto overflow-x-auto no-scrollbar py-1"
        style={{
          maskImage:
            "linear-gradient(to right, transparent, black 4%, black 96%, transparent)",
        }}
      >
        <span className="text-[11px] font-black uppercase tracking-[0.24em] text-muted-foreground whitespace-nowrap pl-1">
          Villes
        </span>
          ...
          {displayCities.map((city) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className={cn(
                  "relative rounded-full px-4 py-2 md:px-5 md:py-2.5 text-xs md:text-sm font-bold transition-all duration-300 whitespace-nowrap flex-shrink-0 font-body overflow-hidden",
                  isActive
                    ? "text-white shadow-lg shadow-primary/20"
                    : "bg-muted/70 text-foreground border border-border/60 hover:border-primary/40 hover:text-primary shadow-sm",
                )}
              >
                {isActive && (
                  <motion.div
