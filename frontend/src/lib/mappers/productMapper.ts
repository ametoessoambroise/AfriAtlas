...
function primaryProductImageUrl(
  ...
  return primary
    ? resolveImageUrl(primary.url, primary.storage_provider)
    : PLACEHOLDER_IMAGE;
}
...
export function mapNestedProductToMenuItem(
  p: PlaceNestedProductResponse,
): MenuItem {
  const popular =
    p.tags?.some((t) => /populaire|chef|signature/i.test(t)) ?? false;
  return {
    id: p.id,
    name: p.name.trim(),
    price: parseMoney(p.price),
