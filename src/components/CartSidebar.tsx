  ...
  return (
    <div className="flex flex-col h-full bg-card">
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        {onClose && (
          <button onClick={onClose} aria-label="Fermer le panier">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
        ...
        <>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {items.map((ci) => (
              <div
                key={ci.id}
                className="flex items-center gap-3 p-3 bg-muted/50 rounded-xl relative overflow-hidden group"
              >
                <div
                  className={`absolute inset-0 bg-background/50 backdrop-blur-[1px] z-10 flex items-center justify-center transition-opacity ${isBusy ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                >
                ...
                {!ci.is_available && (
                  <div className="absolute inset-0 bg-destructive/5 border border-destructive/20 z-10 p-2 text-xs text-destructive font-bold flex items-end">
                    <span className="bg-destructive text-destructive-foreground px-2 py-0.5 rounded">
                      Rupture
                    </span>
                  </div>
                )}
