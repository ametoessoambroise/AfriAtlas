...
import { CheckCircle, Package, MapPin, Printer, ArrowRight } from "lucide-react";
...
export default function ConfirmationPage() {
  ...
  if (orderQuery.isLoading) return <div className="p-20 text-center font-bold">Génération de votre reçu...</div>;
    ...
    <div className="max-w-3xl mx-auto px-6 py-20">
      <div className="bg-card rounded-[3rem] border border-border shadow-xl overflow-hidden">
        ...
        <div className="bg-primary p-12 text-center text-white">
          ...
          <p className="opacity-80 font-medium">Votre commande #{orderId?.slice(0, 8)} est en cours de préparation.</p>
        </div>
        ...
        <div className="bg-surface-alt p-6 text-center border-t border-border">
           <p className="text-xs text-muted-foreground font-medium">
             Un email de confirmation contenant votre QR Code a été envoyé à votre adresse.
           </p>
        </div>
      </div>
      
      <p className="mt-12 text-center text-xs text-muted-foreground">
        Besoin d'aide ? <span className="text-primary font-bold cursor-pointer">Contactez le Support Client Atlas</span>
      </p>
    </div>
  );
}
