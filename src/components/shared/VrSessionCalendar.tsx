...
export function VrSessionCalendar({ placeSlug, onSuccess, className }: VrSessionCalendarProps) {
  const { data: sessions, isLoading: sessionsLoading } = useVrSessions(placeSlug);
  ...
  const bookMutation = useBookVrSession(placeSlug, selectedSessionId || "");
  ...
  if (!sessions?.length) {
    return (
      <div className="p-8 text-center bg-zinc-900/40 border border-white/5 rounded-2xl">
        <div className="mb-4 inline-flex p-3 rounded-full bg-white/5">
          <CalendarIcon className="h-6 w-6 text-white/20" />
        </div>
        <p className="text-white/40 text-sm">Aucune session VR disponible pour cet établissement.</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-6", className)}>
      {/* Session Selection */}
      <section>
        <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-3">Expérience VR</h4>
          ...
          {sessions.map((session) => (
            ...
            >
              <div>
                ...
                <div className="text-[10px] opacity-60">{session.duration_minutes} min · {session.price} {session.currency}</div>
              </div>
              {selectedSessionId === session.id && <Check className="h-4 w-4 text-primary" />}
            </button>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Calendar */}
        <section>
          <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-3">Choisir la date</h4>
        ...
        {/* Slots */}
        <section>
          <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-3">Créneaux horaires</h4>
          ...
          <div className="mt-6 p-4 rounded-2xl bg-zinc-900/40 border border-white/5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold w-4 text-center">{participants}</span>
                <button 
                   onClick={() => setParticipants(Math.min(selectedSession?.max_participants || 4, participants + 1))}
                   className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10"
                >
                  +
                </button>
      ...
      <div className="pt-4 border-t border-white/5">
        <Button
          ...
          disabled={bookMutation.isPending || !selectedSlot || !selectedSessionId}
          ...
          {bookMutation.isPending ? (
            ...
            <div className="flex items-center justify-between w-full px-4">
              ...
              <div className="flex items-center gap-2 text-sm uppercase tracking-widest opacity-80">
                <span>{selectedSession ? (parseFloat(selectedSession.price) * participants) : 0} FCFA</span>
